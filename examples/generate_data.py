...
def save_housing_data(points_per_party, folder_party_data, folder_dataset):

    # dataset_folder 'examples/datasets'
    # party_folder 'examples/data/mnist/random'

    import random

    learners_num = len(points_per_party)
    train_samples_per_learner = points_per_party[0]
    test_samples = points_per_party[0]
    ...
    train_data, test_data = housing_np[:required_training_samples], housing_np[-test_samples:]
    # First n-1 are input features, last feature is prediction/output feature.
    x_train, y_train = train_data[:, :-1], train_data[:, -1:]
    x_test, y_test = test_data[:, :-1], test_data[:, -1:]

    x_chunks, y_chunks = np.split(x_train, learners_num), np.split(y_train, learners_num)

    for cidx, (x_chunk, y_chunk) in enumerate(zip(x_chunks, y_chunks)):
        np.savez(str(folder_party_data + "/data_party{}.npz".format(cidx)), x_train=x_chunk,
                 y_train=y_chunk, x_test=x_test, y_test=y_test)

    print('Finished! :) Data saved in ', folder_party_data)
...
if __name__ == '__main__':
    ...
    elif dataset == 'housing_mlp':
        save_housing_data(points_per_party, folder_party_data, folder_dataset)
    else:
